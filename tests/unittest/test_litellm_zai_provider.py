"""
Tests for Z.AI (Zhipu AI GLM) provider wiring in LiteLLMAIHandler.__init__.

Verifies that ZAI.KEY / ZAI.API_BASE settings are exported to the env vars
LiteLLM's 'zai/' provider reads (ZAI_API_KEY / ZAI_API_BASE), and that nothing
is exported when they are unset.
"""
import os
...
def _isolate_env(monkeypatch):
    ...
    for var in ("ZAI_API_KEY", "ZAI_API_BASE"):
        os.environ.pop(var, None)


def test_zai_env_vars_exported_from_settings(monkeypatch):
    overrides = {
        "ZAI.KEY": "zai-test-123",
        "ZAI.API_BASE": "https://open.bigmodel.cn/api/paas/v4",
    }
    monkeypatch.setattr(litellm_handler, "get_settings", lambda: _make_settings(overrides))

    litellm_handler.LiteLLMAIHandler()

    assert os.environ["ZAI_API_KEY"] == "zai-test-123"
    assert os.environ["ZAI_API_BASE"] == "https://open.bigmodel.cn/api/paas/v4"


def test_zai_env_vars_absent_when_unset(monkeypatch):
    monkeypatch.setattr(litellm_handler, "get_settings", lambda: _make_settings({}))

    litellm_handler.LiteLLMAIHandler()

    assert "ZAI_API_KEY" not in os.environ
    assert "ZAI_API_BASE" not in os.environ


def test_zai_api_base_optional(monkeypatch):
    """API base is optional (defaults to LiteLLM's built-in api.z.ai endpoint)."""
    overrides = {"ZAI.KEY": "zai-only-key"}
    monkeypatch.setattr(litellm_handler, "get_settings", lambda: _make_settings(overrides))

    litellm_handler.LiteLLMAIHandler()

    assert os.environ["ZAI_API_KEY"] == "zai-only-key"
    assert "ZAI_API_BASE" not in os.environ
